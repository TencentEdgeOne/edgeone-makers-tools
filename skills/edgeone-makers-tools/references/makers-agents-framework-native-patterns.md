...
> For the actual Makers patterns you should write, see `makers-agents-node-frameworks-langgraph.md`, `makers-agents-node-frameworks-claude-sdk.md`, and `makers-agents-platform-node-entry.md`.
...
> **→ How the Makers version differs**: see `makers-agents-node-frameworks-langgraph.md`; the model goes through the gateway (`context.env`), tools come from `context.tools.all()` (with `agents.framework='deepagents'` or `'langgraph'`), and memory reuses the LangGraph adapters (direct properties `langgraphCheckpointer` / `langgraphStore`).
