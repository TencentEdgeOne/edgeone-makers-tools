...
import { resolveModelName, collectGatewayEnv } from '../_model';  // see makers-agents-platform-node-entry.md §3
...
5. **Sandbox** — if you use `context.sandbox`, its `/tmp` is per-request and easily lost; keep a process-level file cache and re-upload every request. See [makers-agents makers-agents-capabilities-sandbox.md](makers-agents-capabilities-sandbox.md).
...
See [makers-agents makers-agents-python-frameworks-claude-sdk.md](makers-agents-python-frameworks-claude-sdk.md).
