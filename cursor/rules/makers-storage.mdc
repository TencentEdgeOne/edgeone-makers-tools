...
validate:
  - pattern: "context\\.env\\.[A-Za-z_]*[Kk][Vv]"
    message: "KV is a console-bound global variable, not on context.env — call my_kv.get(...) directly. See makers-storage-kv.md."
metadata:
  author: edgeone
  version: "1.0.0"
---
...
>
> 🔒 **Every Blob store: `getStore({ name, consistency: "strong" })` — never the string form `getStore("name")`.** The string form defaults to eventual consistency and causes "I just wrote but the next read returns stale/null" bugs. Full rule and rationale: [makers-storage-blob.md](makers-storage-blob.md) → IRON RULE (top of file).
...
│   → KV Storage (global variable)           → read makers-storage-kv.md
└─ Cloud Function (Node.js, has npm)?  ← default for any real backend
    ├─ Files / images / large objects?
    │   → Blob Storage                       → read makers-storage-blob.md
    └─ Structured data (records, lists, counters, per-user state)?
        → Blob with setJSON / key prefixes   → read makers-storage-blob.md
```
...
| Task | Read |
|------|------|
| KV Storage (Edge Functions, global variable, put/get/delete/list) | [makers-storage-kv.md](makers-storage-kv.md) |
| Blob Storage (Cloud Functions, npm SDK, file upload/download, pre-signed URLs) | [makers-storage-blob.md](makers-storage-blob.md) |
