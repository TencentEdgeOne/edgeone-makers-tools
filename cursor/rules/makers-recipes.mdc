...
> 💾 **Where does the data live?** This platform has **no database**. The API skeletons above return empty data — to actually persist records, uploads, votes, or per-user state, back them with **Blob**. See the recipe below and [makers-storage → Blob as your backend](makers-storage-blob.md).
...
Swap the key scheme for other shapes: `users/<uid>.json` for save-state, `counts/<option>.json` (strong consistency) for votes, `uploads/<id>.jpg` + `items/<id>.json` for file uploads. Full patterns: [makers-storage → Blob as your backend](makers-storage-blob.md).
...
⚠️ **Prerequisites**: You must enable KV Storage in the console and bind a namespace first. See [makers-storage-kv.md](makers-storage-kv.md)
