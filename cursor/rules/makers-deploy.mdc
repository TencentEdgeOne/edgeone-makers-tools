...
| `Failed to create pages project` (dev) / `Makers project exceeds 40 limit` (deploy) | Account hit the **40-project cap**. Both dev and deploy fail the same way. Hand back to the user; the agent MUST NOT delete projects itself (CLI has no delete command by design — do not route around via HTTP APIs or cached credentials). User's options: ① delete an unused project in the EdgeOne Makers console; ② name an existing linked project via `-n <existing-project-name>`. |
